...
/**
  * This is an event sourced entity. It has a state, [[BmapState]], which
  ...
  */
class BmapEntity extends PersistentEntity {

  override type Command = BmapCommand[_]
  override type Event = BmapEvent
  override type State = BmapState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: BmapState = BmapState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case BmapState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        BmapState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class BmapState(message: String, timestamp: String)
...
object BmapState {
  /**
    ...
    */
  implicit val format: Format[BmapState] = Json.format
}

/**
  * This interface defines all the events that the BmapEntity supports.
  */
sealed trait BmapEvent extends AggregateEvent[BmapEvent] {
  def aggregateTag: AggregateEventTag[BmapEvent] = BmapEvent.Tag
}

object BmapEvent {
  val Tag: AggregateEventTag[BmapEvent] = AggregateEventTag[BmapEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends BmapEvent
...
/**
  * This interface defines all the commands that the BmapEntity supports.
  */
sealed trait BmapCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends BmapCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends BmapCommand[String]
...
/**
  ...
  * application loader.
  */
object BmapSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[BmapState]
  )
}
